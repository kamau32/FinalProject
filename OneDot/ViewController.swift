...
fileprivate extension ViewController {
    ...
    //called when we need to see if we have collided
    func checkCollision() {
        enemyViews.forEach {
    ...
    //sends out the alert when u die
    
    func displayGameOverAlert() {
        let (title, message) = getGameOverTitleAndMessage()
        let alert = UIAlertController(title: "You died", message: message, preferredStyle: .alert)
    ...
    //options for when the game ends
    
    func getGameOverTitleAndMessage() -> (String, String) {
        let elapsedSeconds = Int(elapsedTime) % 60
        switch elapsedSeconds {
        case 0..<10: return ("New Game", "🥉")
        case 10..<30: return ("New Game", "🥈")
        case 30..<60: return ("New Game", "🥇")
        default:
            return ("", "")
        }
    }
